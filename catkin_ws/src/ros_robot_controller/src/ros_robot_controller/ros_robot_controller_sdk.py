...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(implement control function through serial port)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED控制(LED control)
    PACKET_FUNC_BUZZER = 2  # 蜂鸣器控制(buzzer control)
    PACKET_FUNC_MOTOR = 3  # 电机控制(motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM servos control, which are number 1 to 4 from inside to the outside of the expansion board)
    PACKET_FUNC_BUS_SERVO = 5  # 总线舵机控制(bus servo control)
    PACKET_FUNC_KEY = 6  # 按键获取(obtain button)
    PACKET_FUNC_IMU = 7  # IMU获取(obtain IMU)
    PACKET_FUNC_GAMEPAD = 8  # 手柄获取(obtain gamepad)
...
class PacketReportKeyEvents(enum.IntEnum):
    # 按键的不同状态(different status of the buttons)
...
class Board:
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(The subcommand 0x01 is used to set the SSID. The second byte is the length of the string, which includes the '\0' string termination character)
