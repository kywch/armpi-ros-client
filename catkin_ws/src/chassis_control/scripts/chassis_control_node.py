...
class EncoderMotorController:
    ...
    def set_speed(self, speed, motor_id=None, offset=0):
        global th
        # 通过IIC发布控制信息到电机驱动板(publish control information to motor driver board via IIC)
        with smbus2.SMBus(self.i2c_port) as bus:
            try:
                if motor_id is None:
...
# 麦轮子底盘速度处理(mecanum chassis speed precessing)
class MecanumChassis:
    ...
    # 控制XY方向平移函数(function for controlling XY directional translation)
...
last_angular = 0
last_direction = 90
# 缓变速处理函数(function for smooth velocity control)
...
# 平移控制回调函数(callback function for translation control)
...
# 普通控制回调函数(callback function for ordinary control)
def Set_Velocity(msg):
    ...
    if th is None: # 通过子线程去控制缓变速(control smooth velocity through sub-thread)
